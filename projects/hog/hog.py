    ...
    for _ in range(num_rolls):
        ...
        if not one_appeared:
            if current_num == 1:
                ...
                score += current_num
    return 1 if one_appeared else score
    # END PROBLEM 1
...
def picky_piggy(score):
    ...
    """
    # BEGIN PROBLEM 2
    sequence = int(round(1 / 7, 6) * 10 ** 6)
    index = 6 - score % 6
    while index >= 0:
        digit, sequence, index = sequence % 10, sequence // 10, index - 1
    return digit if digit != 0 else 7
    # END PROBLEM 2
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            score0 += hog_pile(score0, score1)  # add additional score due to hog_pile
        else:
            num_roll1 = strategy1(score1, score0)
            score1 += take_turn(num_roll1, score0, dice, goal)
            score1 += hog_pile(score1, score0)  # add additional score due to hog_pile
        who = next_player(who)
    # END PROBLEM 5
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(current_score0, current_score1):
        current_score = (current_score0 if who == 0 else current_score1)
        current_gain = current_score - last_score
        if current_gain > running_high:
            print(current_gain, "point(s)! That's a record gain for Player", str(who) + "!")
        return announce_highest(who, current_score, max(current_gain, running_high))
    return say
    # END PROBLEM 7
