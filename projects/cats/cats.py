...
def about(topic):
    ...
    def select(p):
        for word in p.split():
            if remove_punctuation(word.lower()) in topic:
                return True
        return False
    return select
    # END PROBLEM 2
...
def accuracy(typed, reference):
    ...
    # BEGIN PROBLEM 3
    correct_count, typed_len, refer_len = 0, len(typed_words), len(reference_words)
    if typed_len == 0 and refer_len == 0:
        return 100.0
    if typed_len == 0 or refer_len == 0:
        return 0.0
    for i in range(min(typed_len, refer_len)):
        if typed_words[i] == reference_words[i]:
            correct_count += 1
    return correct_count / typed_len * 100
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    CHARACTER_COUNT, SEC_PER_MIN = 5, 60
    word_count = len(typed) / CHARACTER_COUNT
    min_count = elapsed / SEC_PER_MIN
