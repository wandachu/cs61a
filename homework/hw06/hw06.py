...
class VirFib():
    ...
    def next(self):
        ...
        if self.value <= 0:
            self.prev_one = 1
        new_fib = VirFib(self.prev_one + self.value)
        new_fib.prev_one = self.value
        return new_fib
...
def is_bst(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    # Check if the current node has at most 2 children
    if t.is_leaf():
        return True
    if len(t.branches) > 2:
        return False
    # Two cases: when only one child (could be left or right) and when two children
    # Case 1:
    left_child = t.branches[0]
    if len(t.branches) == 1:  # only has one child, could be left or right
        return is_bst(left_child) and (t.label <= bst_min(left_child) or t.label >= bst_max(left_child))
    # Case 2:
    right_child = t.branches[1]
    return is_bst(right_child) and is_bst(left_child) and t.label >= bst_max(left_child) and t.label <= bst_min(right_child)

def bst_min(t):
    """Return the minimum lable of the tree."""
    if t.is_leaf():
        return t.label
    return min(t.label, min([b.label for b in t.branches]))

def bst_max(t):
    if t.is_leaf():
        return t.label
    return max(t.label, max([b.label for b in t.branches]))
